import torch
from transformers import RobertaTokenizer, RobertaModel, AutoProcessor, AutoImageProcessor,ElectraTokenizer, ElectraModel , DebertaV2TokenizerFast, DebertaV2Model
import librosa
import cv2
import numpy as np
import av
...
def make_batchs(sessions):
    ...
    sublabel_list = ['negative','neutral','positive']

    batch_input, batch_audio, batch_video, batch_labels, batch_sublabels = [], [], [], [], []
    max_length = 400000
    relation_ids = []
    for session in sessions:
        ...
        for turn, line in enumerate(session):
            speaker, utt, video_path , emotion, sentiment = line
        ...
        wav_path = video_path.replace("MELD.Raw", "MELD.Wav").replace(".mp4", ".wav")
        ...
        prompt = ("Now"+' <s' + str(now_speaker+1) + '> '
                 + "seems to hold a" + roberta_tokenizer.mask_token + "attitude toward what was said,"
                 + " and externally feels" + roberta_tokenizer.mask_token + ".")
        ...
        sublabel_ind = sublabel_list.index(sentiment)
        batch_labels.append(label_ind)
        batch_sublabels.append(sublabel_ind)
    ...
    batch_sublabels = torch.tensor(batch_sublabels)
    
    return batch_input_tokens, batch_attention_masks, batch_audio, batch_video, batch_labels, batch_sublabels
